...
This is the Code used to generate figures 5 and 6 in Stuart et al. 2018
...
Load in the data from HTseq-count
```{r data}
directory = "../data"
...
write_csv(complete_unnormalized_data, path = "../results/RNA_Seq_Script_Output/complete_unnormalized_counts.csv")
(complete_unnormalized_data / colData(dds)$sizeFactors) %>% write_csv(path = "../results/RNA_Seq_Script_Output/complete_normalized_counts.csv")
```
...
Perform principal component analysis to plot the first two components
```{r pca}
...
dat = data.frame(pc1 =pca$x[,1], pc2 = pca$x[,2], Timepoint = group$Timepoint, names = substr(colnames(rld),1,5))
